...
def perf_eval(H, Py_p, nnrate, K, var_noise=1):
    ...
    np.save('./data/'+'nnrate_Qos.npy', nnrate)
    ...
    linestyle = ['-', '--', ':', '-.']
    label = ['WMMSE', 'ePCNet', 'MAX Power', 'Random Power']
    ...
    plt.savefig('CDF_%d.eps' % K, format='eps', dpi=2000)
    plt.show()
    return 0
